locals {
  ...
  pg_family = var.parameter_group_family != null ? var.parameter_group_family : "postgres${split(".", var.engine_version)[0]}"
}
...
resource "aws_db_parameter_group" "postgresql" {
  ...
  # Enable pg_stat_statements if requested
  dynamic "parameter" {
    for_each = var.enable_pg_stat_statements ? [1] : []
    content {
  ...
  # Set track_activity_query_size for better query monitoring
  parameter {
...
resource "null_resource" "create_newrelic_user" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      # Create New Relic user and grant permissions
      PGPASSWORD='${var.create_random_password ? random_password.master[0].result : var.master_password}' psql \
        -h ${aws_db_instance.postgresql.address} \
        -p ${aws_db_instance.postgresql.port} \
        -U ${var.master_username} \
        -d ${var.database_name} \
        -c "CREATE USER ${var.newrelic_user} WITH PASSWORD '$NR_PASSWORD';"
      ...
      PGPASSWORD='${var.create_random_password ? random_password.master[0].result : var.master_password}' psql \
        ...
        -U ${var.master_username} \
        -d ${var.database_name} \
        -c "GRANT pg_read_all_stats TO ${var.newrelic_user};"
      ...
      PGPASSWORD='${var.create_random_password ? random_password.master[0].result : var.master_password}' psql \
        ...
        -U ${var.master_username} \
        -d ${var.database_name} \
        -c "GRANT CONNECT ON DATABASE ${var.database_name} TO ${var.newrelic_user};"
      
      # Enable pg_stat_statements if requested
      if [ "${var.enable_pg_stat_statements}" = "true" ]; then
        PGPASSWORD='${var.create_random_password ? random_password.master[0].result : var.master_password}' psql \
          -h ${aws_db_instance.postgresql.address} \
          -p ${aws_db_instance.postgresql.port} \
          -U ${var.master_username} \
        ...
        # Grant permissions to view statements
        PGPASSWORD='${var.create_random_password ? random_password.master[0].result : var.master_password}' psql \
          -h ${aws_db_instance.postgresql.address} \
          -p ${aws_db_instance.postgresql.port} \
          -U ${var.master_username} \
          -d ${var.database_name} \
          -c "GRANT SELECT ON pg_stat_statements TO ${var.newrelic_user};"
      fi
